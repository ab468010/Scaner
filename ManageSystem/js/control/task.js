...
function Page(page) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i in s) {
                var estart = new Date(s[i].EstimatedStart).Format('yyyy-MM-dd hh:mm');
                var eend = new Date(s[i].EstimatedEnd).Format('yyyy-MM-dd hh:mm');
                var astart = Globals.datetime_is_null(s[i].ActualStart);
                var aend = Globals.datetime_is_null(s[i].ActualEnd);
                var con = "<td>" + s[i].TaskName + "</td><td>" + s[i].ProjectName + "</td><td>" + s[i].RoomName + "</td><td>" + estart +
