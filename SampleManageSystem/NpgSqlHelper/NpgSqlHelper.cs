...
namespace SqlHelper
{
    ...
    {
        //获取PostgreSQL数据库连接字符串，其属于静态变量且只读，项目中所有文档可以直接使用
        public static readonly string ConnectionString = System.Configuration.ConfigurationManager.AppSettings["NpgConnectionString"];
        ...
        /// 执行查询，返回DataSet
        /// </summary>
        public static DataSet ExecuteQuery(string connectionString, CommandType cmdType, string cmdText, params NpgsqlParameter[] commandParameters)
        {
            using (NpgsqlConnection conn = new NpgsqlConnection(connectionString))
            {
