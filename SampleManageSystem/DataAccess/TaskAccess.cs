...
namespace DataAccess
{
     public  class TaskAccess:IDataAccess.ITaskAccess
    {
        ...
        public IList<Task> SelectTask(int number)
        {
            ...
            string st = @"select taskid, ta.name taskname,pr.name projectname,ro.name roomname,COALESCE(estimatedstart,'1900-1-1') estimatedstart,COALESCE(estimatedend,'1900-1-1') estimatedend,COALESCE(actualstart,'1900-1-1') actualstart,COALESCE(actualend,'1900-1-1') actualend
                from (dbo.task ta inner join dbo.project pr on ta.projectid=pr.projectid) inner join dbo.room ro on ta.roomid=ro.roomid limit 10 offset @number";
            NpgsqlParameter[] par = new NpgsqlParameter[]
            {
            ...
            using(NpgsqlDataReader drt = NpgSqlHelper.ExecuteReader(NpgSqlHelper.ConnectionString, CommandType.Text, st,par))
            {
                while (drt.Read())
                {
                    ta.Add(new Task(drt.GetInt32(0), drt.GetString(1), drt.GetString(2), drt.GetString(3), drt.GetDateTime(4), drt.GetDateTime(5), drt.GetDateTime(6), drt.GetDateTime(7)));
                }
            }
            return ta;
        }
